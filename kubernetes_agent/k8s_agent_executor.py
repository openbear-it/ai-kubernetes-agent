...
class k8sAgentExecutor(AgentExecutor):
    ...
    # Constants for timeout and performance monitoring
    STEP_TIMEOUT = 180  # Maximum time allowed for a single step (seconds)
    SLOW_STEP_THRESHOLD = 90  # Time threshold to mark a step as slow (seconds)
    MAX_TOTAL_TIME = 600  # Maximum total execution time (seconds)
    PROGRESS_INTERVAL = 3  # Send progress message every 3 seconds to keep queue alive
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        - Provides detailed progress updates and performance monitoring
        - Handles timeouts and slow responses gracefully
        """
        ...
        try:
            ...
            progress_interval = self.PROGRESS_INTERVAL  # Send progress messages more frequently
            step_count = 0
            ...
            async for step in self.agent.stream(user_question, selected_tools=[]):
                ...
                step_elapsed = current_time - last_step_time
                last_step_time = current_time
                
                # Log step timing for monitoring
                self.logger.debug(f"Step {step_count} took {step_elapsed:.2f}s")
                if step_elapsed > self.SLOW_STEP_THRESHOLD:  # Log warning for slow steps
                    self.logger.warning(f"Step {step_count} was slow: {step_elapsed:.2f}s")
                
                # Send periodic progress updates to keep the queue alive
                if current_time - last_progress_time > progress_interval:
                    # Choose progress message based on elapsed time
                    msg_index = min(int(elapsed_time / 30), len(progress_messages) - 1)
                    base_msg = progress_messages[msg_index]
                    
                    # Add detailed status
                    progress_msg = f"{base_msg} (Step {step_count}, {elapsed_time:.1f}s elapsed)"
                    if step.get("tool_name"):
                        progress_msg += f", using {step['tool_name']}"
                    ...
                    try:
                        ...
                        if "Queue is closed" in str(queue_error):
                            self.logger.warning(f"A2A queue closed prematurely after {elapsed_time:.2f}s, storing response for final delivery")
